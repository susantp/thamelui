...
export default function useStyles () {
  ...
  const toggleBtnWidth = open ? ' flex justify-center pt-2 text-white border cursor-pointer  border-slate-700 fixed bottom-0 h-9 bg-slate-400 w-60 z-50' : ' flex justify-center pt-2 text-white border cursor-pointer  border-slate-700 fixed bottom-0 h-9 bg-slate-400 w-24'
  const linkDivStyle = open ? ' flex flex-row items-center cursor-pointer hover:bg-blue-400 hover:text-white  w-full p-4' : 'cursor-pointer hover:bg-blue-400 hover:block w-full p-6 '
